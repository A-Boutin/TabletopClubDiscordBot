...
class TicTacToe:
    ...
    async def game(self, ctx, user):
        ...
        while True:
            ...
            click = await gamemsg.wait_for(check=checkEvent)
    ...
    async def run_game(self, ctx, user):
        while not user:
            ...
            reply = await self.client.wait_for("message", check=lambda msg: msg.author == ctx.author)
...
class TicTacToeCog(commands.Cog):
    ...
    async def tictactoe(self, ctx, user: discord.Member=None):
        await TicTacToe(self.client).run_game(ctx, user)
